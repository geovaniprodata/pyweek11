...
def ver_empresa(request, id):
    if request.method == 'GET':
        ...
        # TODO: Listar métricas
        metricas = Metricas.objects.filter(empresa=empresa)

        return render(request, 'ver_empresa.html', {'empresa' : empresa, 'documentos' : documentos, 'metricas' : metricas, 'perc_vendido': int(percentual_vendido), 'concretizado': concretizado, 'perc_disponivel': percentual_disponivel})
...
def realizar_proposta(request, id):
    ...
    if total + float(percentual) > empresa.percentual_equity:
        messages.add_message(request, constants.WARNING, 'O percentual solicitado ultrapassa o limite máximo desta empresa.')
